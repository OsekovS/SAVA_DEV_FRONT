...
const PagesBar = (props) => {
    ...
    for (index; index < props.pagination.fromPage+props.pagination.showedPages && index <= props.pagination.lastPage; index++) {
        if(index===props.pagination.currentPage)
            paginationList.push(<li onClick={props.callBack.bind(this, index,props.indexName,props.id)} className="pages-bar__curr-page" key={index}>{index}</li>)
        else paginationList.push(<li onClick={props.callBack.bind(this, index,props.indexName,props.id)} key={index}>{index}</li>)
    }
    if(props.pagination.lastPage>index){
    paginationList.push(<li className="pages-bar__ellipsis" key={index+1}>...</li>)
    paginationList.push(<li onClick={props.callBack.bind(this,props.pagination.lastPage,props.indexName,props.id)} key={index+2}>{props.pagination.lastPage}</li>)
    }
